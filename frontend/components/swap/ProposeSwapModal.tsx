...
interface ProposeSwapModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (proposerGoalId: string, message?: string) => Promise<void>;
...
const NoiseFilter = () => (
  <svg className="absolute inset-0 w-full h-full opacity-[0.03] pointer-events-none">
    ...
    <rect width="100%" height="100%" filter="url(#swap-noise)" />
  </svg>
);

// Swap icon for header
const SwapIcon = () => (
  <svg
    width="24"
    height="24"
...
export function ProposeSwapModal({
  ...
  const canSubmit = selectedGoalId && !isOverLimit && !isLoading;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await onSubmit(selectedGoalId, message.trim() || undefined);
      setSelectedGoalId("");
      setMessage("");
      onClose();
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Failed to propose swap";
      setError(errorMessage);
    }
  };

  // Filter to only show active public/friends goals
  const availableGoals = userGoals.filter(
    (goal) => goal.status === "active" && (goal.visibility === "public" || goal.visibility === "friends")
  );
        ...
        >
          ...
          <motion.div
            ...
            style={{
              ...
              boxShadow:
                "0 0 60px rgba(20, 184, 166, 0.1), 0 25px 50px -12px rgba(0, 0, 0, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.05)",
            }}
          >
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at top, rgba(20, 184, 166, 0.3) 0%, transparent 60%)",
              }}
            />
            ...
            {/* Header with teal gradient glow */}
            <div className="relative px-6 py-4">
              {/* Header bottom border with teal glow */}
              <div
                className="absolute bottom-0 left-4 right-4 h-px"
                style={{
                  background:
                    "linear-gradient(90deg, transparent 0%, rgba(20, 184, 166, 0.4) 30%, rgba(13, 148, 136, 0.6) 50%, rgba(20, 184, 166, 0.4) 70%, transparent 100%)",
              ...
              <div className="flex items-center justify-between">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="p-2 rounded-lg transition-all duration-200 text-slate-400 hover:text-teal-300 hover:bg-teal-500/10 disabled:opacity-50"
                >
                  <svg
              ...
              <div className="relative px-6 py-4 space-y-4">
                  ...
                  {availableGoals.length > 0 ? (
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {selectedGoalId === goal.id && (
                              <motion.div
                                initial={{ scale: 0 }}
                                animate={{ scale: 1 }}
                                className="ml-2 w-5 h-5 rounded-full bg-teal-500 flex items-center justify-center"
                              >
                                <svg
                                  width="12"
                                  height="12"
                    ...
                    >
                      <p className="text-sm text-slate-300">
                        You don&apos;t have any active public goals to offer.
                      </p>
                      <p className="text-xs text-slate-400 mt-1">
                        Create a public goal first to propose a swap.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <motion.div
                    ...
                    animate={{
                      boxShadow: isFocused
                        ? "0 0 20px rgba(20, 184, 166, 0.15), 0 0 40px rgba(20, 184, 166, 0.05)"
                        : "none",
                    }}
                  ...
                  >
                    ...
                    <textarea
                      ...
                      placeholder="Why do you want to swap? What can you offer?"
                      disabled={isLoading}
                      rows={3}
                ...
                <div className="flex items-center justify-end gap-3">
                  ...
                  <motion.button
                    ...
                    style={{
                      ...
                      boxShadow: canSubmit
                        ? "0 4px 15px rgba(20, 184, 166, 0.3), 0 2px 4px rgba(0, 0, 0, 0.2)"
                        : "none",
                    }}
                  >
